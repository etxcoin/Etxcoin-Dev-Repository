...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ETXCOINTEST2\r");
            Send(hSocket, "WHO #ETXCOINTEST2\r");
        } else {
            // randomly join #ETXCOIN00-#ETXCOIN05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ETXCOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ETXCOIN%02d\r", channel_number).c_str());
        }
